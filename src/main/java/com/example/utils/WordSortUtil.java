...
public class WordSortUtil {
     ...
     * @return 清洗后的单词数组
     */
    public static String[] cleanWord(String[] content, Set<String> excludeWorlds) {
        final Set<String> exclude = excludeWorlds == null ? Collections.emptySet() : excludeWorlds;
        Stream<String> stream = Arrays.stream(content);
        List<String> words = stream.parallel().filter(word -> word.trim().length() > 0)
                .filter(word -> !exclude.contains(word))
                .map(word -> cleanWordStart(word))
                .map(word -> cleanWordEnd(word))
